...
class MedVietAnonymizer:
    ...
    def _fake_cccd(self) -> str:
        return f"VNID-{''.join(str(random.randint(0, 9)) for _ in range(12))}"

    def _fake_phone(self) -> str:
        return f"+84{random.choice([3, 5, 7, 8, 9])}{''.join(str(random.randint(0, 9)) for _ in range(8))}"
        ...
        if strategy == "replace":
            ...
            operators = {
                "DEFAULT": OperatorConfig(
                    "mask", {"masking_char": "*", "chars_to_mask": 6, "from_end": False}
                )
            }
        elif strategy == "hash":
            operators = {
