...
def build_vietnamese_analyzer() -> AnalyzerEngine:
    ...
    # SĐT VN: 0[3|5|7|8|9] + 8 chữ số
    phone_recognizer = PatternRecognizer(
        supported_entity="VN_PHONE",
        patterns=[
            Pattern(
                name="vn_phone",
                regex=r"\b(?:0?[35789]\d{8})\b",
                score=0.85,
            )
        ],
    ...
    # Custom PERSON recognizer để tăng độ phủ tên tiếng Việt trong dataset
    person_recognizer = PatternRecognizer(
        supported_entity="PERSON",
        patterns=[
            Pattern(
                name="vn_person_name",
                regex=r"\b[A-ZÀÁẢÃẠĂẮẰẲẴẶÂẤẦẨẪẬĐEÈÉẺẼẸÊẾỀỂỄỆIÌÍỈĨỊOÒÓỎÕỌÔỐỒỔỖỘƠỚỜỞỠỢUÙÚỦŨỤƯỨỪỬỮỰYỲÝỶỸỴ][\wÀ-ỹ]{1,}(?:\s+[A-ZÀ-Ỵ][\wÀ-ỹ]{1,}){1,3}\b",
                score=0.6,
            )
        ],
        context=["bệnh nhân", "ông", "bà", "bs", "bác sĩ", "tên"],
        supported_language="vi",
    )
